...
mf <- read.csv("../../RD/metaframework.csv")
mf <- mf[which(mf$label_Framework == "Gully CO"),]
mf_ind <- read.csv("../../RD/mf_indicators.csv")|>
  left_join(mf, by=c("Objective"="label_Objective"))

mf_ind <- mf_ind[which(mf_ind$Objective == "Minimize harmful impacts from human activities on cetacean populations and their habitats;" ),]
